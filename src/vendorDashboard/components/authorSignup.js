...
const AuthorSignup = () => {
    ...
    return ( 
        <>
        { companyInfo && <div className=" w-full">
            ...
            <div className=' w-full mt-4 lg:overflow-y-scroll lg:h-[260px]'>
                ...
                <p className=' font-Outfit text-base font-medium text-[#000000B2] mt-4'>Password Strength Requirement</p>
                <div className=' flex flex-row justify-between items-center mt-4 w-full'>
                    {req.map((item, index) =>(
                        <span key={index} className=' flex flex-col text-center items-center'>
                            <p className=' font-Outfit text-lg font-medium text-[#01A85D]'>{ item.example }</p>
                            <p className=' font-Outfit text-sm text-[#12121266] font-medium'>{ item.name }</p>
                        </span>
                    ))}
                </div>
                <span className=' flex flex-row items-center space-x-3 mt-6'>
                    <p className=' font-Outfit text-base font-medium'>Referral Code (optional)</p>
                    <img src={ down } alt="" />
                </span>
            </div>

            <div className=' mt-16 lg:mt-0 lg:absolute bottom-0 w-full left-0 lg:px-10'>
                <Link to='/vendorDashboard'><button className=' w-full bg-[#2F52FF] rounded-[10px] flex items-center justify-center  h-[38px] text-white text-center font-Outfit text-base'>Proceed</button></Link>
                <Link to='/vendorlogin'><p className=' mt-[19px] font-Outfit font-medium text-xs text-[#12121266] text-center'>Already have an Account? <span className=' text-[#2F52FF]'>Login</span></p></Link>
            </div>
        </div>}

        { addInfo && <div className=' w-full'>  
            <img src={ progress } className=' w-full mt-3' alt="" />
            <p className=' font-Outfit font-medium text-2xl mt-6'>Personal Information</p>
            ...
            <div className=' w-full mt-4 lg:overflow-y-scroll lg:h-[260px]'>
                ...
                <label htmlFor="Email" className=' flex flex-col w-full font-Outfit text-sm font-medium mt-4'>
                    Are you affiliated with any publishing house or an independent author?
                    <select type="email" className=' border border-[#EAEBF0] h-[40px] p-2.5 font-Outfit text-sm rounded-[15px] mt-2'>
                ...
                <label htmlFor="Email" className=' flex flex-col w-full mb-12 font-Outfit text-sm font-medium mt-4'>
                    Account Name
                    <input type="email" className=' border border-[#EAEBF0] h-[40px] p-2.5 font-Outfit text-sm rounded-[15px] mt-2' />
                </label>
            </div>
            ...
            <div className=' mt-16 lg:mt-0 lg:absolute bottom-0 w-full left-0 lg:px-10'>
                <button onClick={proceed}  className=' w-full bg-[#2F52FF] rounded-[10px] flex items-center justify-center  h-[38px] text-white text-center font-Outfit text-base'>Proceed</button>
                <Link to='/vendorlogin'><p className=' mt-[19px] font-Outfit font-medium text-xs text-[#12121266] text-center'>Already have an Account? <span className=' text-[#2F52FF]'>Login</span></p></Link>
            </div>
        </div>}
        </>
