    ...
    <>
      <div className=" py-8 md:py-16 px-4 md:px-20 w-full flex flex-col items-center justify-center">
        <p className=" text-center font-Outfit font-semibold text-2xl md:text-[40px] md:leading-[60px]">
          Unlimited Access To Our Vast E-library{" "}
        </p>
        ...
        <div className="flex w-full flex-row items-center justify-start md:justify-center mt-8 overflow-auto">
          ...
          {Array.from(new Set(categories.map((category) => category.tag))).map(
            (tag, index) => (
              <button
                key={index}
        ...
        <div className=" w-full mt-8 grid grid-cols-4 lg:grid-cols-7 gap-[3%]">
          {filteredCategories.map((item, index) => (
            <div key={index} className=" w-full mt-4">
              ...
              <p className=" text-[#000000CC] font-Outfit text-[9px] md:text-xs mt-[6px]">
                {item.date}
              </p>
            </div>
          ))}
        ...
        <div className=" w-full flex justify-center items-center">
          <button className=" mt-[160px] lg:mt-[72px] rounded-[5px] px-6 py-3 font-Outfit text-base font-semibold bg-[#0530A1] shadow-sm shadow-[#1018280A] text-[#fff] text-center">
            View all
          </button>
        </div>
      </div>
