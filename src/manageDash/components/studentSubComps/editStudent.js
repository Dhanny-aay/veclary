...
} from "../../../controllers/schoolControllers/studentController";
...
const EditStudent = ({ triggerFetch, studentID, setEditStudent }) => {
  ...
  const fetchStudentByID = async () => {
    try {
      const data = await handleGetStudentById(studentID);
      if (data) {
        setStudent(data);
      }
    } catch (error) {
  ...
  const handleSave = () => {
    if (validateFields()) {
      ...
      handleUpdateStudentById(studentID, userData, onSuccess, onError);
    }
  };
          ...
          <div className="w-full h-full bg-[#fff] overflow-auto">
            ...
            {loading ? (
              ...
              <>
                ...
                <label className="w-full flex flex-col mt-3 text-[#272D37] font-Outfit font-medium text-sm">
                  Email
                  <input
                    type="text"
                    name="email"
