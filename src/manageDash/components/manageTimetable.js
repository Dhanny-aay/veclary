...
const ManageTimetable = () => {
    ...
    return ( 
        <>
        <div onClick={()=>{setSidebarVisible(false)}} className=" absolute lg:left-[20%] px-6 top-[56px] w-full pb-6 lg:w-[80%]">
            <span onClick={() => handleClick('Home')} className=' cursor-pointer mt-6 flex flex-row items-center'>
                <img src={ arrowBlue } alt="" />
                <p className=' font-Outfit text-[#2F52FF] text-sm font-medium'>Back</p>
                <p className=' font-Outfit text-xl font-semibold mb-2 ml-3'>Timetable</p> 
            </span>

            <div className=' w-full md:items-end flex flex-wrap md:flex-row mt-6 justify-between'>
                <label htmlFor="Class Teacher" className=' font-Outfit flex flex-col w-[48%] md:w-[130px] text-[#272D37] text-xs font-medium'>
                    Timetable
                ...
                <span className=" flex mt-6 md:mt-0 items-start">
                    <button className=' text-center  text-sm font-Outfit font-medium text-white bg-[#2F52FF] py-2 px-3 md:px-6 rounded-[10px]'>Edit Timetable</button>
                </span>
            </div>
                    ...
                    <thead>
                        <tr>
                            ...
                            {daysOfWeek.map((day, index) => (
                                <th key={index} className="border-b px-4 py-2 md:py-6 bg-[#BADAFE] text-black border-[#EAEBF0] font-semibold text-xs font-Outfit text-center">{day}</th>
                            ))}
                        </tr>
                    </thead>
                    <tbody>
                        {timeSlots.map((timeSlot, index) => (
                            <tr key={index}>
                                <td className="border-y border-r border-[#EAEBF0] p-3 md:p-6 text-xs text-black bg-[#BADAFE] font-semibold font-Outfit text-center">{timeSlot}</td>
                                {daysOfWeek.map((day, dayIndex) => (
                                    <td key={`${index}-${dayIndex}`} className="border-y border-[#EAEBF0] px-2 text-xs font-normal font-Outfit text-center">
