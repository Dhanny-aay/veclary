...
const TeacherSchedule = () => {
    ...
    const upcoming = [
        { color:'#006531', time: '08:00AM - 10:00AM', activity: 'Morning Exercise' },
                    ...
                    <div className=" flex mt-3 w-full border-y border-[#9292921A]">
                        <div className="w-16 border-r border-[#9292921A]">
                            {timetableData.map((item, index) => (
                                <div key={index} className="flex items-center justify-center h-16 border-b border-[#9292921A]">
                                    <p className="text-center text-xs py-6 text-[#929292] font-Outfit font-normal">{item.time}</p>
                                </div>
                            ))}
                        </div>
                        <div className="flex-1 w-full">
                            {timetableData.map((item, index) => (
                                <div key={index} style={{ backgroundColor: item.color ? `${item.color}2a` : 'transparent', borderColor: item.color ? `${item.color}` : 'transparent' }} className={`flex ml-1 backdrop-opacity-60 items-center text-left w-full border-l-4 bg-[#0000002a] h-16 justify-center`}>
                        ...
                        {upcoming.map((item, index) => (
                            <div key={index} className=" w-full pl-1 py-2 bg-[#F8F8F8] rounded-[8px]">
                                <div style={{borderColor: item.color}} className=" border-l-4">
                                    <p className=" font-Outfit text-[8px] ml-2 font-normal text-black">Event</p>
                                    <p className=" ml-2 font-medium mt-1 text-xs font-Outfit text-black">{ item.activity }</p>
                                    <p className=" ml-2 font-medium mt-1 text-[10px] font-Outfit text-black">{ item.time }</p>
                                </div>
                            </div>
                        ))}
                        </div>
