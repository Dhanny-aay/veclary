...
const TeacherHeadbar = () => {
    ...
    return ( 
        ...
        <div className=" w-full lg:w-[80%] fixed top-0 lg:left-[20%] h-[56px] px-8 py-3 z-[99] border-b bg-[#fff] border-[#EAEBF0] flex flex-row justify-between items-center ">
            <div>
                <img src={ burger } className=' w-6 z-[9999] lg:hidden' alt=""  onClick={toggleSidebar} />
            </div>
            <div className=" flex items-center space-x-6">
                ...
                <img src={ noti } alt="" />
                <span onClick={() => handleClick('Profile')} className=" cursor-pointer w-[40px] h-[40px] rounded-[50%] bg-[#EAEBF0]"></span>
            </div>
        </div>
        </>
     );
