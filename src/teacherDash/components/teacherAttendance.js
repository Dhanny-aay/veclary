...
const TeacherAttendance = () => {
    ...
    const students = ['John Doryre', 'Jane Doe', 'Alice', 'Bob', 'Charlie', 'David', 'Eva', 'Frank'];

    const weeks = ['1st Week', '2nd Week', '3rd Week', '4th Week', '5th Week', '6th Week'];
    ...
    return ( 
        ...
        <div onClick={()=>{setSidebarVisible(false)}} className=" absolute lg:left-[20%] top-[56px] w-full pb-6 lg:w-[80%]">
            <span onClick={() => handleClick('Home')} className=' cursor-pointer px-6 mt-6 flex flex-row items-center'>
                ...
                <p className=' font-Outfit text-xl font-semibold mb-2 ml-3'>Attendance</p> 
            </span>
                        ...
                        <table className="table-auto border-collapse font-Outfit text-[#5F6D7E]">
                            <thead>
                                <tr>
                                    ...
                                    <th className="border-b border-[#EAEBF0] px-8 py-2">Student Name</th>
                                    {weeks.map((week, index) => (
                                        <th key={index} className="border-b border-[#EAEBF0] px-4 py-2">{week}</th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {students.map((student, studentIndex) => (
                                    <tr key={studentIndex}>
                                        <td className="border-y border-[#EAEBF0] px-4 py-2">{(studentIndex + 1).toString().padStart(3, '0')}</td> 
                                        ...
                                        {weeks.map((week, weekIndex) => (
                                            <td key={weekIndex} className="border-y border-[#EAEBF0] px-4 py-2">
                                                <div className=" flex flex-row">
                                                {days.map((day, dayIndex) => (
                                                    <label key={dayIndex} className=" flex flex-col items-center">
                                                        <span className=" ml-2 text-[#929292] text-xs">{day}</span>
